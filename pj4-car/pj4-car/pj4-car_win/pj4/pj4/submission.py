...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                true_distance = math.sqrt(pow(util.colToX(col)-agentX,2)+pow(util.rowToY(row)-agentY,2))
                p = util.pdf(true_distance,Const.SONAR_STD,observedDist)
    ...
    def elapseTime(self):
        ...
        for (old,new) in self.transProb:
            p = self.belief.getProb(old[0],old[1])
            new_belief.addProb(new[0],new[1],p * self.transProb[(old,new)])
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for tile in self.particles.keys():
            true_distance = math.sqrt(pow(agentX - util.colToX(tile[1]), 2) + pow(agentY - util.rowToY(tile[0]), 2))
            p = util.pdf(true_distance, Const.SONAR_STD, observedDist) * self.particles[tile]
            weightDict[tile] = p
        new_particles = collections.Counter()
        for i in range(self.NUM_PARTICLES):
            element = util.weightedRandomChoice(weightDict)
            new_particles[element] += 1
    ...
    def elapseTime(self):
        ...
        for tile in self.particles.keys():
            for i in range(self.particles[tile]):
                ...
                new_element = util.weightedRandomChoice(weightDict)
                new_particles[new_element] += 1
        self.particles = new_particles
        # END_YOUR_CODE
